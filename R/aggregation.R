...
prepare_sample_data <- function(N = 100,
  ...
  if (!is.null(no_prog_results)) {
    ...
    tmp1 <- append_fun(
      as.data.frame(tmp1), no_prog_tmb_fit, populations,
      type = type
    )
  } else {
    tmp1 <- NULL
  ...
  if (!is.null(prog_results)) {
    ...
    tmp2 <- append_fun(
      as.data.frame(tmp2), prog_tmb_fit, populations,
      type = type
    )
  } else {
    tmp2 <- NULL
...
aggregate_sample_age_group <- function(results_list,
  ...
  results <- setcolorder(results, c(other_names, num_cols))
