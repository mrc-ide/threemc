...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SPARSE_MATRIX(X_spacetime_tmc); // Design matrix for the interaction random effects in the traditionalcircumcision hazard rate
  ...
  PARAMETER_ARRAY(u_spacetime_tmc);
  ...
  PARAMETER(logsigma_spacetime_tmc);      Type sigma_spacetime_tmc      = exp(logsigma_spacetime_tmc);
  ...
  // need autocorrelation paraeter for time TMC
  ...
  ////////////////////////////////////////////
  // AR1 Process 
  nll += GMRF(Q_time)(u_time_mmc);
  // nll += GMRF(Q_time)(u_time_tmc); 
  ...
  // Prior on the TMC logit autocorrelation parameter
  ...
  nll += SEPARABLE(GMRF(Q_space), GMRF(Q_time))(u_spacetime_tmc);
  ...
  for (int i = 0; i < u_spacetime_tmc.cols(); i++) {
    nll -= dnorm(u_spacetime_tmc.col(i).sum(), Type(0), Type(0.001) * u_spacetime_tmc.col(i).size(), true);
  }
  ...
  vector<Type> u_spacetime_tmc_v(u_spacetime_tmc);
  ...
  nll -= dexp(sigma_spacetime_tmc, Type(1), TRUE) + logsigma_spacetime_tmc;
				  ...
				  X_age_mmc * u_age_mmc * sigma_age_mmc + 
					  X_agetime_mmc * u_agetime_mmc_v * sigma_agetime_mmc + 
						  X_agespace_mmc * u_agespace_mmc_v * sigma_agespace_mmc + 
							  ...
							  X_spacetime_tmc * u_spacetime_tmc_v * sigma_spacetime_tmc;
