...
Type threemc(
    ...
    // indicators
    int paed_age_cutoff, // Should we split MMC rates between constant paed and varying adult components?
  ...
  // initialise paedaitric sigma parameters; give value if required
  Type sigma_age_mmc_paed;
  Type sigma_space_mmc_paed;
  Type sigma_agespace_mmc_paed;
  if (paed_age_cutoff == 1) {
    sigma_age_mmc_paed      = exp(logsigma_age_mmc_paed);
    sigma_space_mmc_paed    = exp(logsigma_space_mmc_paed);
    sigma_agespace_mmc_paed = exp(logsigma_agespace_mmc_paed);
  }
  ...
  Type rho_mmc_paed_age1; // = Type(0);
  Type rho_mmc_paed_age2; // = Type(0);
  if (paed_age_cutoff == 1) {
    rho_mmc_paed_age1 = geninvlogit(logitrho_mmc_paed_age1,  Type(-1.0), Type(1.0));
    rho_mmc_paed_age2 = geninvlogit(logitrho_mmc_paed_age2,  Type(-1.0), Type(1.0));
  }
...
///////////////////////////////////////////////
  ...
  // vector<Type> u_agespace_mmc_paed_v;
  // if (paed_age_cutoff == 1) {
  ...
  if (paed_age_cutoff == 1) {
    haz_mmc += X_fixed_mmc_paed * u_fixed_mmc_paed +
