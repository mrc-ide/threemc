...
#' ## Load source code
# source(here::here("code/0_source.R"))

# TEMP:load package functions until they are properly uploaded
f <- list.files("~/imperial_repos/threemc/R")
f <- paste0("~/imperial_repos/threemc/R/", f)
invisible(lapply(f, source))

# TEMP: Changing directory
setwd("~/imperial_repos/circumcision-coverage/")

# Path to TMB cpp model
model_path <- "Runs/"
...
# levels of area hierarchy for each country
# area_lev_df <- read_csv(here::here("data/datapack_psnu_area_level.csv"))
# pull value for current target country
# area_lev <- area_lev_df$psnu_area_level[area_lev_df$iso3 == cntry]

# TEMP: set area level to 1
area_lev <- 1
...
filters <- c("iso3" = "LSO", sex = "male")
# areas <- read_circ_data(here::here("data/areas.geojson"), filters)
areas <- read_circ_data("data/areas.geojson", filters)
# survey_clusters <- read_circ_data(here::here("data/survey_clusters.csv"),
survey_clusters <- read_circ_data("data/survey_clusters.csv",
                                  filters)
# survey_individuals <- read_circ_data(here::here("survey_individuals.csv.gz"),
survey_individuals <- read_circ_data("data/survey_individuals.csv.gz",
                                     filters)
# survey_clusters <- read_circ_data(here::here("survey_circumcision.csv.gz"),
survey_circumcision <- read_circ_data("data/survey_circumcision.csv.gz",
                                      filters)
...
# compile TMB model
compile_tmb(paste0(model_path, "Surv_SpaceAgeTime_ByType.cpp"))

# dyn.load(dynlib(here::here("Runs/Surv_SpaceAgeTime_ByType")))
dyn.load(dynlib("Runs/Surv_SpaceAgeTime_ByType"))
...
# sample from TMB fit
fit <- circ_sample_tmb(obj, opt, n_samples = 1000)
...
data.table::fwrite(out, file = here::here(
  paste0("Runs/", cntry, "_Results_DistrictAgeTime_ByType.csv.gz")
))
save(fit, file = here::here(
  paste0("Runs/", cntry, "_TMBObjects_DistrictAgeTime_ByType.RData")
))
