...
1. Finish write up (pretty much done)
2. Fix plots 
3. Add references to (1) Matt's paper and (2) the TMB paper
...
cumulative incidence (i.e. circumcision coverage). (REFERENCE THREEMC)
...
<!-- `threemc` is implemented in `TMB`, which maximises the Laplace approximation  -->
<!-- of the marginal likelihood using automatic differentiation (AD) on the joint  -->
<!-- likelihood. `TMB` models can be optimised with R's built in optimisers, such  -->
<!-- as `stats::nlminb`. `TMB` is particularly fast in comparison to other methods  -->
<!-- when there are many random effects, such as are commonly found in  -->
<!-- spatio-temporal models such as `threemc`, and also allows a lot of flexibility  -->
<!-- in model specification thorugh it's use of a C++ template function.  -->
<!-- (REFERENCE) (Could word better) -->

<!-- `threemc` is implemented in `TMB`, which contructs an objective function in  -->
<!-- the negative joint log-likelihood ??? -->

`threemc` is implemented in Template Model Builder (`TMB`) (REFERNCE).
...
estimate posterior distributions for model parameters. (REFERENCE MATT) 
...
such as are commonly found in spatio-temporal models like `threemc`. (REFERENCE 
TMB comparison paper).
...
Next, set the following metadata (to avoid "magic numbers") in our analysis:
```{r metadata}
cntry <- "MWI"          # iso3 code for country to model 
cens_age <- 59          # max circ. age to include in model, censor older ages
N <- 1000               # N samples drawn from post. predictive distribution
...
hyperparameters. (REFERENCE Naomi)
...
Be aware that this can take quite a long amount of time. Just think of all 
...
We can see that, by and large, MC rates are highest among those aged around 
10-13. Circumcisions do not seem to occur for those over 30s.
In many areas (such as Lilongwe), circumcisions do not seem occur in 
any great numbers. Perhaps this is because circumcision is already high in 
these regions. However, this may be unlikely, as traditional circumcision 
(the main reason for high circumcision in Malawi pre-2012 (the year they began 
implementing Voluntary Male Medical Circumcision (VMMC) programmes REFERENCE)) 
is generally low in these regions (this can easily be plotted using 
`rate_tmc`). Also, circumcision rates have increased quite significantly in 
many regions since 2008. In some (such as Blantyre City and Machinga), MC rates 
have slowed since 2015 to 2021 (for which we have no survey data and must 
forecast out-of-sample), indicating that circumcision coverage may be quite 
have become quite high started "tapering-off" there.
...
the age-distribution of coverage in many of these same regions, which seems to 
"peak" at slightly older teenage years (around 17-19).
In Lilongwe and other regions with "flat" MC rates in our previous plots, we 
unfortunately learn that coverage is not high.
Circumcision coverage is greater than or the same in every region from 
2008 to 2015 and from 2015 to 2021. In those regions where we observed some 
"tapering off" in circumcision rates, we can see that coverage is indeed high. 
In particular, Machinga and Mangochi have had consistently high coverage across 
our predictive period, suggesting that TMC is high there.
Blantyre City and Lilongwe City, which have high HIV burdens (REFERENCE) and 
had very low coverage in 2008, have seen significant (forecasted) increases in 
circumcision levels, highlighting the work being done in resource allocation, 
which motivates this model (this perhaps also suggests a greater ease in 
implementing circumcision programmes in urban areas).
